...
NumberRec::NumberRec(QObject *parent) : QObject(parent)
{

}

QStringList NumberRec::TrainFromFile(QString TrainingSetPath, QString NNPath)
{
    QStringList Log;
    ...
    for(int i = 0; i < 100; i++)
    {
        ...
        QString str;
        Log.append(str.sprintf("error %.12f :: learning rate %f", (double) error / data.rows, (double) rate));
        rate *= anneal;
    }

    // This is how you save the neural network to disk.
    xtsave(tinn, NNPath.toStdString().c_str());
    xtfree(tinn);
    dfree(data);
...
QStringList NumberRec::Recognize(QString TrainingSetPath, QString NNPath)
{
    QStringList StrOut;
    // Input and output size is harded coded here as machine learning
    // repositories usually don't include the input and output size in the data itself.
    ...
    const Tinn loaded = xtload(NNPath.toStdString().c_str());
